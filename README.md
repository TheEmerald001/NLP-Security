...
This tool allows you to identify vulnerabilities in system and software designs by analysing language patterns using NLP algorithms, and mapping these vulnerabilitiies to the CAPEC standards, allowing for proofing and patching of such vulnerabilities.
## Prerequisites (Libraries Required)
* Ensure to install all requires libraries.
...
* This graph will show where the coherence values reaches a peak. Observe and write down at what point the graph is at its second peak. (Check for where the graph increases reaches a maximum then decreases and increases again then the max point before the second decrease is the number of topics that you will want to remember.)
